...
public class ShareHomo extends JFrame implements ActionListener {
  ...
  public void txtShare() 
  {
    //YOU WILL WRITE YOUR CODE HERE
	  //originaltext numbers entered in the box
	  int a1=5;
	  int a2=6;
	  int prime = 137;
	  int fx = 0;
	  String astring=originaltxt.getText();
	  String[] text1 = astring.split("\n");
	  for(int i=0; i<text1.length;i++) {
		  for (int j = 1;j<=5;j++) {
			  fx = (Integer.valueOf(text1[i]) + a1 * j + a2 * j * j) % prime;
			  if (j == 1) {
				  share1.append(String.valueOf(fx));
				  share1.append("\n");
			  }
			  if (j == 2) {
				  share2.append(String.valueOf(fx));
				  share2.append("\n");
			  }
			  if (j == 3) {
				  share3.append(String.valueOf(fx));
				  share3.append("\n");
			  }
			  if (j == 4) {
				  share4.append(String.valueOf(fx));
				  share4.append("\n");
			  }
			  if (j == 5) {
				  share5.append(String.valueOf(fx));
				  share5.append("\n");
			  }
		  }
	  }
  ...
  {
  //YOU WILL WRITE YOUR CODE HERE
	  String temp=share1.getText();
	  String[] text1 = temp.split("\n");
	  temp=share2.getText();
	  String[] text2 = temp.split("\n");
	  temp=share3.getText();
	  String[] text3 = temp.split("\n");
	  temp=share4.getText();
	  String[] text4 = temp.split("\n");
	  temp=share5.getText();
	  String[] text5 = temp.split("\n");
	  ...
	  int prime = 137;
	  for (int i=0 ;i<text1.length;i++) {
		  Avg1 = (Avg1 + Integer.valueOf(text1[i])) % prime;
		  Avg2 = (Avg2 + Integer.valueOf(text2[i])) % prime;
		  Avg3 = (Avg3 + Integer.valueOf(text3[i])) % prime;
		  Avg4 = (Avg4 + Integer.valueOf(text4[i])) % prime;
		  Avg5 = (Avg5 + Integer.valueOf(text5[i])) % prime;
	  }
	  share1.append(String.valueOf((Avg1 * return_inverse(text1.length,prime)) % prime ));
	  share2.append(String.valueOf((Avg2 * return_inverse(text1.length,prime)) % prime ));
	  share3.append(String.valueOf((Avg3 * return_inverse(text1.length,prime)) % prime ));
	  share4.append(String.valueOf((Avg4 * return_inverse(text1.length,prime)) % prime ));
	  share5.append(String.valueOf((Avg5 * return_inverse(text1.length,prime)) % prime ));
	  //System.out.println("Share 1 is " +share1);
  }
  
  public int return_inverse (int a, int b) {
	  BigInteger aa = new BigInteger(String.valueOf(a));
	  BigInteger bb = new BigInteger(String.valueOf(b));
	  BigInteger inv;
	  inv = aa.modInverse(bb);
	  return Integer.valueOf(inv.toString());
  }
  ...
  //YOU WILL WRITE YOUR CODE HERE
	  int prime = 137;
	  int reconstructavg=0;
	  int reconstructavg1=0;
	  int reconstructavg2=0;
	  int reconstructavg3=0;
	  int x=0;
	  int x1=1;
	  int x2=2;
	  int x3=3;
	  ...
	  int y2=Integer.parseInt(text2[text1.length-1]);
	  int y3=Integer.parseInt(text3[text1.length-1]);
	  reconstructavg1=y1*(((x-x2)*(x-x3))* return_inverse((x1-x2)*(x1-x3) , prime));
	  reconstructavg2=y2*(((x-x1)*(x-x3))* return_inverse((x2-x1)*(x2-x3),prime ));
	  reconstructavg3=y3*(((x-x1)*(x-x2))* return_inverse((x3-x1)*(x3-x2), prime ));
	  reconstructavg=(reconstructavg1+reconstructavg2+reconstructavg3)% prime;
	  //share1.append(String.valueOf(reconstructavg1 * return_inverse((x1-x2)*(x1-x3) , prime)));
	  //share2.append(String.valueOf(reconstructavg2 * return_inverse((x2-x1)*(x2-x3),prime )));
	  //share3.append(String.valueOf(reconstructavg3 * return_inverse((x3-x1)*(x3-x2), prime )));
	  
	  retxt.setText(String.valueOf(reconstructavg));
